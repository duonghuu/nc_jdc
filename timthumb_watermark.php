...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if(! preg_match('/^image\/(?:gif|jpg|jpeg|png|JPG|PNG)$/i', $mimeType)){
		...
		if(preg_match('/^image\/(?:jpg|jpeg|JPG)$/i', $mimeType)){ 
			$imgType = 'jpg';
	...
	protected function serveExternalImage(){
		...
		if(! preg_match("/^image\/(?:jpg|jpeg|gif|png|JPG|PNG)$/i", $mimeType)){
	...
	protected function imagecreatefromfile($image_path) {
	...
	protected function openImage($mimeType, $src){
    	...
    	$watermark = $this->imagecreatefromfile('upload/watermark.png');
 		...
 		$tam = imagesx($watermark)/imagesy($watermark);
		...
		$rong = $dai/$tam;
 		$watermark_pos_x = (imagesx($image)/2) - ($dai / 2);
  		$watermark_pos_y = imagesy($image) - $rong;

  imagecopyresized($image, $watermark,  $watermark_pos_x, $watermark_pos_y, 0, 0, $dai, $rong, imagesx($watermark), imagesy($watermark));
