...
function get_mans(){
	...
	$sql = "select ten_vi, id, stt, hienthi, id_list, id_cat, id_item, tieubieu, noibat, id_sub, banchay, title, keywords, description, icon from $where $limit";
	$d->query($sql);
...
function save_man(){
	...
	$file_name2=images_name($_FILES['fileicon']['name']);
	...
	if($id){
		...
		if($photo2 = upload_image("fileicon", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_baiviet,$file_name2)){
			$data['icon'] = $photo2;	
			$d->setTable('baiviet');
			$d->setWhere('id', $id);
			$d->select();
		...
		$data['id_length'] = (int)$_POST['id_length'];
		$data['id_themes'] = (int)$_POST['id_themes'];
		...
		$data['tab2'] = magic_quote($_POST['tab2']);
		$data['tab3'] = magic_quote($_POST['tab3']);
		$data['tab4'] = magic_quote($_POST['tab4']);
		...
		$data['solieu'] = str_replace(',','',$_POST['solieu']);

		$data['tenlienhe'] = $_POST['tenlienhe'];
		$data['sdtlienhe'] = $_POST['sdtlienhe'];
...
function delete_list(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id,photo,thumb from #_baiviet_list where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			while($row = $d->fetch_array()){
				delete_file(_upload_baiviet.$row['photo']);
