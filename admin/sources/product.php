...
switch($act){
	...
	case "man_thuonghieu":
		get_thuonghieus();
		$template = "product/man_thuonghieu/items";
		break;
	case "add_thuonghieu":		
		$template = "product/man_thuonghieu/item_add";
		break;
	...
	case "man_tinhtrang":
		get_tinhtrangs();
		$template = "product/man_tinhtrang/items";
		break;
	case "add_tinhtrang":		
		$template = "product/man_tinhtrang/item_add";
		break;
	case "edit_tinhtrang":		
		get_tinhtrang();
		$template = "product/man_tinhtrang/item_add";
	...
	case "man_kieu":
		get_kieus();
		$template = "product/man_kieu/items";
		break;
	case "add_kieu":		
		$template = "product/man_kieu/item_add";
		break;
	case "edit_kieu":		
		get_kieu();
		$template = "product/man_kieu/item_add";
	...
	case "man_gia":
		get_man_gias();
		$template = "product/man_gia/items";
		break;
	case "add_gia":		
		$template = "product/man_gia/item_add";
		break;
	case "edit_gia":		
		get_man_gia();
		$template = "product/man_gia/item_add";
...
function get_mans(){
	...
	if($_REQUEST['id_thuonghieu']!='')
	{
		$where.=" and id_thuonghieu = ".$_GET['id_thuonghieu'];
		$link_add .= "&id_thuonghieu=".$_GET['id_thuonghieu'];
	}
	...
	$sql = "select id, ten_vi, stt, hienthi, id_list, id_cat, noibat, id_item, id_sub, sp_banchay, sp_khuyenmai, sp_moi from $where $limit";
	$d->query($sql);
...
function save_man(){
	...
	if($id){
		...
		$data['id_tinhtrangmay'] = (int)$_POST['id_tinhtrangmay'];	
		$data['id_kieu'] = (int)$_POST['id_kieu'];	
		$data['id_donvi'] = (int)$_POST['id_donvi'];
		...
		$data['khuyenmai_vi'] = magic_quote($_POST['khuyenmai_vi']);
		$data['khuyenmai_en'] = magic_quote($_POST['khuyenmai_en']);
		...
		$data['nhanhapkhau'] = $_POST['nhanhapkhau'];
		...
		$data['h1'] = magic_quote($_POST['h1']);
		$data['h2'] = magic_quote($_POST['h2']);
		$data['h3'] = magic_quote($_POST['h3']);
		$data['h4'] = magic_quote($_POST['h4']);
		$data['h5'] = magic_quote($_POST['h5']);
		$data['h6'] = magic_quote($_POST['h6']);
...
function get_lists(){
	...
	$sql = "select ten_vi,id,stt,hienthi,noibat,trangtrong from $where $limit";
	$d->query($sql);
	$items = $d->result_array();
...
function save_list(){
	...
	if($id){
		...
		if($photo1 = upload_image("file1", 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_product,$file_name1)){
			$data['icon'] = $photo1;
			$d->setTable('product_list');
			$d->setWhere('id', $id);
			$d->select();
...
function delete_thuonghieu(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id from #_thuonghieu where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
...
function get_tinhtrangs(){
	...
	{
	$id_up = $_REQUEST['hienthi'];
	$sql_sp = "SELECT id,hienthi FROM table_tinhtrangmay where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	$hienthi=$cats[0]['hienthi'];
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_tinhtrangmay SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_tinhtrangmay SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}	
	}
	#-------------------------------------------------------------------------------
...
function get_tinhtrang(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", $_SESSION['links_re']);	
	$sql = "select * from #_tinhtrangmay where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", $_SESSION['links_re']);
	$item = $d->fetch_array();
}
...
function delete_tinhtrang(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id from #_tinhtrangmay where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			$sql = "delete from #_tinhtrangmay where id='".$id."'";
			$d->query($sql);
		}
		if($d->query($sql))
			redirect($_SESSION['links_re']);
...
function get_kieus(){
	...
	{
	$id_up = $_REQUEST['hienthi'];
	$sql_sp = "SELECT id,hienthi FROM table_kieu where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	$hienthi=$cats[0]['hienthi'];
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_kieu SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_kieu SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}	
	}
	#-------------------------------------------------------------------------------
...
function get_kieu(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", $_SESSION['links_re']);	
	$sql = "select * from #_kieu where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", $_SESSION['links_re']);
	$item = $d->fetch_array();
}
...
function delete_kieu(){
	global $d;
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id from #_kieu where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			$sql = "delete from #_kieu where id='".$id."'";
			$d->query($sql);
		}
		if($d->query($sql))
			redirect($_SESSION['links_re']);
...
function get_man_gias(){
	...
	$sql = "select id, ten_vi, giatu, giaden, donvigiatu, donvigiaden, hienthi, stt from $where $limit";
	$d->query($sql);
	$items = $d->result_array();
...
function get_man_gia(){
	global $d, $item;

	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
	transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_gia");
...
function save_man_gia(){
	...
	if($id){
		...
		$data['donvigiatu'] = (int)$_POST['donvigiatu'];
		$data['giaden'] = str_replace(',','',$_POST['giaden']);
		$data['donvigiaden'] = (int)$_POST['donvigiaden'];
		$data['stt'] = $_POST['stt'];
		$data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;
		$data['ngaysua'] = time();
		$d->setTable('gia');
