...
function get_items(){		
	...
	if($_GET["sotien"]!='' && $_GET["sotien"]>0){
		$sql="select giatu,giaden from #_gia where id='".$_GET["sotien"]."'";
		$d->query($sql);
	...
	$url_link='index.php?com=order&act=man&keyword='.$_GET['keyword'].'&ngaybd='.$_GET['ngaybd'].'&ngaykt='.$_GET['ngaykt'].'&sotien='.$_GET['sotien'].'&httt='.$_GET['httt'].'&tinhtrang='.$_GET['tinhtrang'];		
}
