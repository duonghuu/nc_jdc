<?php
    ...
    $d->reset();
    $sql = "select trinhduyet,count(trinhduyet) as dem from counter group by trinhduyet";
    $d->query($sql);
    $row_trinhduyet = $d->result_array();

    $d->reset();
    $sql = "select hdh2,count(hdh2) as dem from counter group by hdh2 order by hdh2 asc";
    $d->query($sql);
    $row_hdh2 = $d->result_array();

    $d->reset();
    $sql = "select thietbi,count(thietbi) as dem from counter group by thietbi order by thietbi asc";
    $d->query($sql);
    $row_thietbi = $d->result_array();

    $d->reset();
    $sql = "select thietbi,count(thietbi) as dem from counter group by thietbi order by thietbi asc";
    $d->query($sql);
    $row_tb = $d->result_array();

    $d->reset();
    $sql = "select quocgia,count(quocgia) as dem from counter where quocgia!='' group by quocgia order by dem desc limit 0,10";
    $d->query($sql);
    $row_quocgia = $d->result_array();

    $d->reset();
    $sql = "select website,count(website) as dem from counter where website!='' group by website order by dem desc limit 0,8";
    $d->query($sql);
    $row_dentu = $d->result_array();
        ...
        <div class="row_30" style="padding-left:20px;">
            <table width="100%" border="1" class="table_tk">
                ...
                <?php for($i=0;$i<count($row_trinhduyet);$i++){ ?>
                    <tr>
                        <td><?=$row_trinhduyet[$i]['trinhduyet']?></td>
                        <td><?=$row_trinhduyet[$i]['dem']?></td>
                        <td><?=phantram($row_trinhduyet[$i]['dem'],$tong) ?> %</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
        ...
        <div class="row_30" style="padding-left:20px;">
            <table width="100%" border="1" class="table_tk">
                ...
                <?php for($i=0;$i<count($row_hdh2);$i++){ ?>
                    <tr>
                        <td><?=$row_hdh2[$i]['hdh2']?></td>
                        <td><?=$row_hdh2[$i]['dem']?></td>
                        <td><?=phantram($row_hdh2[$i]['dem'],$tong) ?> %</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
            ...
            <table width="100%" border="1" class="table_tk">
                ...
                <?php for($i=0;$i<count($row_dentu);$i++){ ?>
                    <tr>
                        ...
                        <td><?=phantram($row_dentu[$i]['dem'],$tong) ?> %</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
        ...
        <div class="row_30" style="padding-left:20px;">
            <table width="100%" border="1" class="table_tk">
                ...
                <?php for($i=0;$i<count($row_thietbi);$i++){ ?>
                    <tr>
                        <td><?=$row_thietbi[$i]['thietbi']?></td>
                        <td><?=$row_thietbi[$i]['dem']?></td>
                        <td><?=phantram($row_thietbi[$i]['dem'],$tong) ?> %</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
...
<script>
        ...
        option = {
            ...
            legend: {
                ...
                '<?=$row_trinhduyet[$i]['trinhduyet']?>',
                <?php } ?>
                '<?=$row_trinhduyet[0]['trinhduyet']?>'
                ]
            },
            ...
            series: [
            {
                    ...
                    data: [
                    <?php for($i=1;$i<count($row_trinhduyet);$i++){ ?>
                        { value: <?=$row_trinhduyet[$i]['dem']?>, name: '<?=$row_trinhduyet[$i]['trinhduyet']?>' },
                    <?php } ?>
                        { value: <?=$row_trinhduyet[0]['dem']?>, name: '<?=$row_trinhduyet[0]['trinhduyet']?>' }
                    ]
                }
            ]
        };
            ...
            series: [
                {
                    ...
                    data: [
                        { value: <?=$row_hdh2[1]['dem']?>, name: '<?=$row_hdh2[1]['hdh2']?>' },
                        { value: <?=$row_hdh2[2]['dem']?>, name: '<?=$row_hdh2[2]['hdh2']?>' },
                        { value: <?=$row_hdh2[4]['dem']?>, name: '<?=$row_hdh2[4]['hdh2']?>' }
                    ]
                },
                {
                    name: 'Access Sources',
                    ...
                    data: [
                    <?php for($i=1;$i<count($row_hdh2);$i++){ ?>
                        { value: <?=$row_hdh2[$i]['dem']?>, name: '<?=$row_hdh2[$i]['hdh2']?>' },
                    <?php } ?>
                        { value: <?=$row_hdh2[0]['dem']?>, name: '<?=$row_hdh2[0]['hdh2']?>' }
                    ]
                }
            ]
        };
        ...
        option = {
            ...
            legend: {
                ...
                <?php for($i=1;$i<count($row_thietbi);$i++){ ?>
                    '<?=$row_thietbi[$i]['thietbi']?>',
                <?php } ?>
                    '<?=$row_thietbi[0]['thietbi']?>'
                ]
            },
            color: ['#EAC435', '#345995', '#E40066', '#03CEA4', '#FB4D3D', '#403F4C', '#FA7921', '#1B998B'],
             ...
             <?php for($i=1;$i<count($row_thietbi);$i++){ ?>
                {
                    ...
                    data: [
                        { name: 'Device', value: <?=round(phantram($tong-$row_thietbi[$i]['dem'],$tong))?>, itemStyle: labelBottom },
                        { name: '<?=$row_thietbi[$i]['thietbi']?>', value: <?=round(phantram($row_thietbi[$i]['dem'],$tong))?>, itemStyle: labelTop }
                    ]
                },
                <?php } ?>
                {
                    ...
                    itemStyle: labelFromatter,
                    data: [
                        { name: 'Device', value: <?=round(phantram($tong-$row_thietbi[0]['dem'],$tong))?>, itemStyle: labelBottom },
                        { name: '<?=$row_thietbi[0]['thietbi']?>', value: <?=round(phantram($row_thietbi[0]['dem'],$tong))?>, itemStyle: labelTop }
                    ]
                }
            ]
        };
            ...
            xAxis: [
                {
                    ...
                    '<?=trim($row_quocgia[$i]['quocgia'])?>'<?php if(($i+1)!=count($row_quocgia)){ echo ",";}?>
                    <?php } ?>
                    ]
                }
            ],
            ...
            series: (function () {
                ...
                for (var i = 0; i < 30; i++) {
                    series.push({
                        ...
                        data: [
                        <?php for($i=0;$i<count($row_dentu);$i++){?>
                            { value: i * 128 + 80, name: '<?=$row_dentu[$i]['website']?>' },
                        <?php } ?>
                        ]
                    })
                }
