...
function fordacap($table1='',$table2='',$table3='',$table4='',$type){
	global $d,$lang,$str;

	$d->reset();
	$sql = "select id, ten_$lang as ten, tenkhongdau from #_$table1 where hienthi=1 and type='$type' order by stt";
	$d->query($sql);
	...
	foreach ($danhmuc_cap1 as $key_list => $value_list) {
		if($table2 != '') {
			$d->reset();
			$sql="select id, ten_$lang as ten, tenkhongdau from #_$table2 where hienthi=1 and type='$type' and id_list='".$value_list["id"]."' order by stt";
			$d->query($sql);
			$danhmuc_cap2=$d->result_array();
		}

		$str.='<li><a href="'.$value_list["tenkhongdau"].'" title="'.$value_list["ten"].'">'.$value_list["ten"].'</a>';

		if($table2 != '' && count($danhmuc_cap2)>0){
			$str.='<ul>';
			foreach ($danhmuc_cap2 as $key_cat => $value_cat) {
				if($table3 != '') {
					$d->reset();
					$sql="select id, ten_$lang as ten, tenkhongdau from #_$table3 where hienthi=1 and type='$type' and id_cat='".$value_cat["id"]."' order by stt";
					$d->query($sql);
					$danhmuc_cap3=$d->result_array();
				}

				$str.='<li><a href="'.$value_cat["tenkhongdau"].'" title="'.$value_cat["ten"].'">'.$value_cat["ten"].'</a>';

				if($table3 != '' && count($danhmuc_cap3)>0){
					$str.='<ul>';
					foreach ($danhmuc_cap3 as $key_item => $value_item) {
						if($table4 != '') {
							$d->reset();
							$sql="select id, ten_$lang as ten, tenkhongdau from #_$table4 where hienthi=1 and type='$type' and id_item='".$value_item["id"]."' order by stt";
							$d->query($sql);
							$danhmuc_cap4=$d->result_array();
						}
						$str.='<li><a href="'.$value_item["tenkhongdau"].'" title="'.$value_item["ten"].'">'.$value_item["ten"].'</a>';
						if($table4 != '' && count($danhmuc_cap4)>0){
							$str.='<ul>';
							foreach ($danhmuc_cap4 as $key_sub => $value_sub) {
								$str.='<li><a href="'.$value_sub["tenkhongdau"].'" title="'.$value_sub["ten"].'">'.$value_sub["ten"].'</a></li>';
							}
							$str.='</ul>';
						}
						$str.='</li>';
	...
	if($type == 'product') {
		$str.='<li><a href="dich-vu" title="'._dichvu.' ++">'._dichvu.' ++</a></li>';
	}
	$str.='</ul>';		
	return $str;
}
...
function pagination($query,$per_page=10,$page=1,$url='?'){   
    ...
    $prevlabel = "&lsaquo; "._trangtruoc;
    $nextlabel = _trangtiep." &rsaquo;";
    $lastlabel = _trangcuoi." &rsaquo;&rsaquo;";
...
class paging_ajax
{
    ...
    public function Load()
    {
        ...
        if ($first_btn && $this->cur_page > 1) {
            $msg .= "<li p='1' class='active'>"._trangdau."</li>";
        } else if ($first_btn) {
            $msg .= "<li p='1' class='$this->class_inactive'>"._trangdau."</li>";
        }
    
        // HIỂN THỊ NÚT PREVIOUS
        if ($previous_btn && $this->cur_page > 1) {
            $pre = $this->cur_page - 1;
            $msg .= "<li p='$pre' class='active'>"._trangtruoc."</li>";
        } else if ($previous_btn) {
            $msg .= "<li class='$this->class_inactive'>"._trangtruoc."</li>";
        }
        ...
        if ($next_btn && $this->cur_page < $no_of_paginations) {
            $nex = $this->cur_page + 1;
            $msg .= "<li p='$nex' class='active'>"._trangsau."</li>";
        } else if ($next_btn) {
            $msg .= "<li class='$this->class_inactive'>"._trangsau."</li>";
        }
        
        // HIỂN THỊ NÚT LAST
        if ($last_btn && $this->cur_page < $no_of_paginations) {
            $msg .= "<li p='$no_of_paginations' class='$this->class_active'>"._trangcuoi."</li>";
        } else if ($last_btn) {
            $msg .= "<li p='$no_of_paginations' class='$this->class_inactive'>"._trangcuoi."</li>";
        }
